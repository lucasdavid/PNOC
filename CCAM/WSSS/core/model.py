...
class ResNetSeries(nn.Module):
    def __init__(self, backbone='resnet50', pretrained='supervised', dilated=False):
        ...
        if 'resnest' in backbone:
            ...
            model_fn = getattr(resnest, backbone)
            model = model_fn(pretrained=True, dilated=dilated, dilation=dilation)
            del model.avgpool
            del model.fc
        elif 'resnet' in backbone:
            from . import resnet
            
            model_fn = getattr(resnet, backbone)
            model = model_fn(pretrained=pretrained == 'supervised')
        else:
            raise NotImplementedError

        if pretrained == 'supervised':
            print(f'Loaded imagenet weights')
...
class Network(nn.Module):
    def __init__(self, backbone='resnet50', pretrained='mocov2', cin=None):
        super(Network, self).__init__()

        self.backbone = ResNetSeries(backbone, pretrained)
...
def get_model(backbone, pretrained, cin=2048+1024):
    return Network(backbone, pretrained, cin)
