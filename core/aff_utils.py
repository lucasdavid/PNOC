...
def affinity_sparse2dense(affinity_sparse, ind_from, ind_to, n_vertices):
  ...
  affinity_dense = torch.sparse.FloatTensor(
    ...
    torch.cat([affinity_sparse, torch.ones([n_vertices]), affinity_sparse])
  ).to_dense().cuda()
...
def propagate_to_edge(x, edge, radius=5, beta=10, exp_times=8):
  ...
  sparse_aff = edge_to_affinity(torch.unsqueeze(edge_padded, 0), path_index.path_indices)

  dense_aff = affinity_sparse2dense(sparse_aff, path_index.src_indices, path_index.dst_indices, ver_padded * hor_padded)
...
class GetAffinityLabelFromIndices:
  ...
  def __call__(self, segm_map):
    ...
    neg_aff_m = (~equal_m & valid_m).astype(np.float32)
