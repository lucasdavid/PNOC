...
class Bottle2neck(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, dilation=1, baseWidth=28, scale=4):
    ...
    for i in range(self.nums):
      convs.append(
        nn.Conv2d(width, width, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, bias=False)
      )
...
class DLA(nn.Module):
  ...
  def __init__(
    ...
    self.base_layer = nn.Sequential(
      nn.Conv2d(3, channels[0], kernel_size=7, stride=1, padding=3, bias=False), BatchNorm(channels[0]),
...
def res2net_dla60(pretrained=None, **kwargs):
  ...
  model = DLA([1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024], block=Bottle2neck, **kwargs)
  if pretrained:
...
def res2next_dla60(pretrained=None, **kwargs):
  ...
  model = DLA([1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024], block=Bottle2neckX, **kwargs)
  if pretrained:
