...
class Res2Net(nn.Module):
  ...
  def __init__(
    self, block, layers, baseWidth=26, scale=4, num_classes=1000, strides=(1, 2, 2, 2), norm_layer=nn.BatchNorm2d
  ):
    ...
    self.conv1 = nn.Sequential(
      nn.Conv2d(3, 32, 3, 2, 1, bias=False), norm_layer(32), nn.ReLU(inplace=True),
      nn.Conv2d(32, 32, 3, 1, 1, bias=False), norm_layer(32), nn.ReLU(inplace=True),
  ...
  def _make_layer(self, block, planes, blocks, stride=1):
    ...
    layers.append(
      block(
        self.inplanes, planes, stride, downsample=downsample, stype='stage', baseWidth=self.baseWidth, scale=self.scale
      )
    )
