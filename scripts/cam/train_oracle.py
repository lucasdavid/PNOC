...
if __name__ == '__main__':
  ...
  mask_loss_fn = nn.CrossEntropyLoss(ignore_index=255).to(DEVICE)
  ...
  for step in range(step_init, step_max):
    images, targets, masks = train_iterator.get()
    ...
    # OC-CSE
    labels_mask, labels_indices = occse.split_label(targets, k, choices, focal_factor, args.oc_strategy)
    labels_oc = (targets - labels_mask).clip(min=0)

    o_loss = mask_loss_fn(features, masks)
