...
def _work(process_id, model, dataset, args):
  ...
  with torch.no_grad(), cuda.device(process_id):
    ...
    for iter, pack in enumerate(data_loader):
      ...
      detected = detect_instance(
        rw_up.cpu().numpy(), instance_shape, instance_class_id, max_fragment_size=size[0] * size[1] * 0.01
      )
...
def run(args):
  model = getattr(importlib.import_module(args.irn_network), 'EdgeDisplacement')(model_name=args.model_name)
